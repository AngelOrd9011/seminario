...
const updateMutation = gql`
  mutation MyUpdateMutation($id: Int!, $_set: view_teachers_set_input!) {
    update_view_teachers_by_pk(pk_columns: { id: $id }, _set: $_set) {
      id
      username
    }
  }
...
const insertMutation = gql`
  mutation MyInsertMutation($object: view_teachers_insert_input!) {
    insert_view_teachers_one(object: $object) {
      id
      username
    }
  }
...
const deleteMutation = gql`
  mutation MyDeleteMutation($id: Int!) {
    delete_view_teachers_by_pk(id: $id) {
      id
    }
  }
`;
const deleteManyMutation = gql`
  mutation MyDeleteManyMutation($_in: [Int!]!) {
    delete_view_teachers(where: { id: { _in: $_in } }) {
      affected_rows
    }
  }
`;
export const Profesores = graphql(() => {
  ...
  useEffect(() => {
    ...
    client.query({ query: MY_QUERY_QUERY, ...administratorHeader }).then((data) => {
      let _entities = data?.data?.view_teachers?.map((entity, index) => {
    ...
    if (entity.username) {
      ...
      if (entity.id) {
        updateEntity({
          ...
          const index = findIndexById(data.data.update_view_teachers_by_pk.id);
          _entities[index] = data.data.update_view_teachers_by_pk;
        ...
        addEntity({
          ...
          .then((data) => {
            _entities.push(data.data.insert_view_teachers_one);
            _entities = _entities.map((entity, index) => {
              return { ...entity, numero: index + 1 };
            });
            setEntities(_entities);
    ...
    <div>
      <div className='p-grid'>
        <div className='p-col-12'>
          <Card title='Administración de Profesores' className='card no-gutter widget-overview-box widget-overview-box-1'>
            ...
            {!loading && (
              <>
                ...
                >
                  ...
                  <Column field='username' header='CURP' headerStyle={{ width: '20%' }} />
                  <Column header='Nombre' headerStyle={{width:'30%'}}/>
                  <Column body={actionBodyTemplate} headerStyle={{ width: '20%' }} />
                </DataTable>
              </>
            )}
            <Dialog visible={entityDialog} header='Profesor' modal style={{ width: '50vw' }} className='p-fluid' footer={entityDialogFooter} onHide={hideDialog}>
              <div className='p-field'>
                <label htmlFor='entity'>CURP:</label>
                <InputText
                  id='entity'
                ...
                />
                {submitted && !entity?.username && <medium className='p-invalid'>Se requiere el CURP del profesor.</medium>}
              </div>
            </Dialog>
            <Dialog visible={deleteEntityDialog} style={{ width: '450px' }} header='Confirmación' modal footer={deleteEntityDialogFooter} onHide={hideDialog}>
              <div className='p-d-flex p-ai-center p-jc-center'>
                ...
                {entity && (
                  <span>
                    ¿Estas seguro de borrar este profesor: <b>{entity.numero + '.-' + entity.username}</b>?
                  </span>
                )}
              </div>
            </Dialog>
            <Dialog visible={deleteEntitiesDialog} style={{ width: '450px' }} header='Confirmación' modal footer={deleteEntitiesDialogFooter} onHide={hideDialog}>
              <div className='p-d-flex p-ai-center p-jc-center'>
                <i className='pi pi-exclamation-triangle p-mr-3' style={{ fontSize: '2rem' }} />
                {entity && <span>¿Estas seguro de borrar los profesores seleccionados?</span>}
              </div>
            </Dialog>
          </Card>
        </div>
