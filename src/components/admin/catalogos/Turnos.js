...
const updateMutation = gql`
  mutation MyUpdateMutation($id: Int!, $_set: shifts_set_input!) {
    update_shifts_by_pk(pk_columns: { id: $id }, _set: $_set) {
      id
      shift
    }
  }
...
const insertMutation = gql`
  mutation MyInsertMutation($object: shifts_insert_input!) {
    insert_shifts_one(object: $object) {
      id
      shift
    }
  }
...
const deleteManyMutation = gql`
  mutation MyDeleteManyMutation($_in: [Int!]!) {
    delete_shifts(where: { id: { _in: $_in } }) {
      affected_rows
    }
  }
`;
export const Turnos = graphql(() => {
  ...
  useEffect(() => {
    ...
    client.query({ query: MY_QUERY_QUERY, ...administratorHeader }).then((data) => {
      let _entities = data?.data?.shifts?.map((entity, index) => {
    ...
    if (entity.shift) {
      ...
      if (entity.id) {
        updateEntity({
          ...
          const index = findIndexById(data.data.update_shifts_by_pk.id);
          _entities[index] = data.data.update_shifts_by_pk;
        ...
        addEntity({
          ...
          .then((data) => {
            _entities.push(data.data.insert_shifts_one);
            _entities = _entities.map((entity, index) => {
              return { ...entity, numero: index + 1 };
            });
            setEntities(_entities);
    ...
    <div>
      <div className='p-grid'>
        <div className='p-col-12'>
          <Card title='Administración de Turnos' className='card no-gutter widget-overview-box widget-overview-box-1'>
            ...
            {!loading && (
              <>
                ...
                >
                  ...
                  <Column field='shift' header='Turno' headerStyle={{ width: '30%' }} />
                  <Column body={actionBodyTemplate} headerStyle={{ width: '20%' }} />
                </DataTable>
              </>
            )}
            <Dialog visible={entityDialog} header='Turno' modal style={{ width: '50vw' }} className='p-fluid' footer={entityDialogFooter} onHide={hideDialog}>
              <div className='p-field'>
                <label htmlFor='entity'>Nombre:</label>
                <InputText
                  id='entity'
                  value={entity.shift}
                  onChange={(e) => onInputChange(e, 'shift')}
                  required={true}
                  rows={3}
                  cols={20}
                  className={classNames({
                    'p-invalid': submitted && !entity?.shift,
                  })}
                />
                {submitted && !entity?.shift && <medium className='p-invalid'>Se requiere el nombre del turno.</medium>}
              </div>
            </Dialog>
            <Dialog visible={deleteEntityDialog} style={{ width: '450px' }} header='Confirmación' modal footer={deleteEntityDialogFooter} onHide={hideDialog}>
              <div className='p-d-flex p-ai-center p-jc-center'>
                ...
                {entity && (
                  <span>
                    ¿Estas seguro de borrar este turno: <b>{entity.numero + '.-' + entity.shift}</b>?
                  </span>
                )}
              </div>
            </Dialog>
            <Dialog visible={deleteEntitiesDialog} style={{ width: '450px' }} header='Confirmación' modal footer={deleteEntitiesDialogFooter} onHide={hideDialog}>
              <div className='p-d-flex p-ai-center p-jc-center'>
                <i className='pi pi-exclamation-triangle p-mr-3' style={{ fontSize: '2rem' }} />
                {entity && <span>¿Estas seguro de borrar los turnos seleccionados?</span>}
              </div>
            </Dialog>
          </Card>
        </div>
